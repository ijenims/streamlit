...
def sidebar_fft_settings(usecols: List[int], df: pd.DataFrame) -> Tuple[int, int, dict]:
    with st.sidebar:
...
def show_time_series_plot(df: pd.DataFrame, usecols: List[int], extraction_settings: dict):
    ...
    if extraction_settings["selection_type"] != "全範囲" and extraction_settings["data_points"] is not None:
        ...
        if extraction_settings["selection_type"] == "データ個数":
            ax.text(points/2, ax.get_ylim()[1], f'抽出範囲: {points}点',
...
def main():
    ...
    if uploaded_file:
        ...
        try:
            ...
            # FFT設定と実行
            fft_column, samplerate, extraction_settings = sidebar_fft_settings(usecols, df)
            
            # サンプリングレートを extraction_settings に追加
            extraction_settings["sampling_rate"] = samplerate

            # 時系列データの表示
            if st.session_state.get("show_raw", False):
                show_time_series_plot(st.session_state.df, st.session_state.usecols, extraction_settings)
